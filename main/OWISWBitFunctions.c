...
void __delay_us(uint32_t var) {
    ...
    volatile uint32_t PORT_TIME = OWI_DELAY_OFFSET_CYCLES;
    while (PORT_TIME)
        PORT_TIME--;
}
 ...
 *
 *  \param  pins    A num of the pin to initialize.
 */
void OWI_Init(unsigned char pins)
{
    ...
    __delay_us(OWI_DELAY_H_STD_MODE);
}
 ...
 *  bus.
 *
 *  \param  pins    A num of the pin to write to.
 */
void OWI_WriteBit1(unsigned char pins)
{
    ...
    __delay_us(OWI_DELAY_A_STD_MODE);

    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    __delay_us(OWI_DELAY_B_STD_MODE);
...
void OWI_WriteBit0(unsigned char pins)
{
    ...
    // Drive bus low and delay.
    OWI_PULL_BUS_LOW(pins);
    __delay_us(OWI_DELAY_C_STD_MODE);

    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    __delay_us(OWI_DELAY_D_STD_MODE);
 ...
 *
 *  \param  pins    A num of the pin to read from.
 *
...
unsigned char OWI_ReadBit(unsigned char pins)
{
    ...
    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    __delay_us(OWI_DELAY_E_STD_MODE);

    // Sample bus and delay.
    bitsRead = gpio_get_level(pins);
    __delay_us(OWI_DELAY_F_STD_MODE);
 ...
 *
 *  \param  pins    A num of the pin to send the Reset signal on.
 *
...
unsigned char OWI_DetectPresence(unsigned char pins)
{
    ...
    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    __delay_us(OWI_DELAY_I_STD_MODE);

    // Sample bus to detect presence signal and delay.
    presenceDetected = (gpio_get_level(pins) == 0);
    __delay_us(OWI_DELAY_J_STD_MODE);
